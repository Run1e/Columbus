...
Settings() {
	...
	for i, b in arrMenu
	{
		menu[i] := set.Add("Text", "xp yp+3 w120 h25 0x200 gSelect", "  " b)
		CtlColors.Attach(menu[i], MenuBG, "000000")
	...
	set.Add("Text",, "Fokus Hotkey:")
	fokushwnd := set.Add("Hotkey", "", Settings.Hotkeys.Fokus)
	...
	CtlColors.Change(menu[1], "33b5e5", "F0F0F0")
	...
	if (k ~= "1|2|3|4")
		loop % arrMenu.MaxIndex()
			CtlColors.Change(menu[A_Index], ((k = A_Index) ? "33b5e5" : "F0F0F0"), ((k = A_Index) ? "F0F0F0" : "000000"))
	...
	if (MainHotkey = FokusHotkey) {
		Tray.Tip("Hotkeys have to be unique.")
		return
	}
	...
	Settings.Hotkeys := {Main:MainHotkey, Fokus:FokusHotkey}
